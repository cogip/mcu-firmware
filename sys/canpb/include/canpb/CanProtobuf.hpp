...
///              Each library/application using canpb must define a unique id (32-bit integer
///              of type cogip::canpb::uuid_t) and registrer a callback function to send
...
/// This wrapper is used to call the message_reader() function from CanProtobuf class
/// from C context, passing the CanProtobuf instance pointer as first parameter.
void *message_reader_wrapper(
    void *arg                                     ///< [in] pointer to CanProtobuf instance
    );
...
public:
    ...
    /// Class constructor.
    explicit CanProtobuf(
        uint8_t can_interface_number    ///< [in] CAN device
        );
