...
int main(void)
{
    ...
    ProfileFeedforwardControllerParameters profile_params(
        10.0,  // max_speed (mm/period)
    ...
    for (int period = 0; period < 150 && !robot.target_reached(); period++) {
        ...
        // Clear new_target flag after first iteration
        if (period == 0) {
            io.set("new_target", false);
        }
    }
