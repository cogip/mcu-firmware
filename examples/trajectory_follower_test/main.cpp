...
{
  public:
    SimplePID(double kp, double ki, double kd) : kp_(kp), ki_(ki), kd_(kd), integral_(0.0), previous_error_(0.0)
    {
    }
...
int main(void)
{
    ...
    // Generate profile for 1000mm movement
    double target_distance = 1000.0;
    double max_speed = 10.0;       // mm/period
    double acceleration = 1.0;     // mm/period²
    double deceleration = 1.0;     // mm/period²

    uint32_t total_periods = profile.generate_optimal_profile(
        robot.velocity(),  // initial velocity
        target_distance,
        acceleration,
        deceleration,
        max_speed,
    ...
    std::cout << "Period | Position (mm) | Velocity (mm/period) | FF Velocity | FB Correction | Distance Remaining\n";
    std::cout << "-------|---------------|----------------------|-------------|---------------|-------------------\n";
