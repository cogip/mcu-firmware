...
class Motor : public PositionalActuator
{
  ...
  protected:
    ...
    motion_control::SpeedLimitFilter* tracker_speed_limit_filter_ = nullptr;

    /// Acceleration filter - safety clamp on speed_order increase (tracker chain, optional).
    motion_control::AccelerationFilter* tracker_acceleration_filter_ = nullptr;

    /// Deceleration filter - reduces speed based on remaining distance (tracker chain, optional).
    motion_control::DecelerationFilter* tracker_deceleration_filter_ = nullptr;
