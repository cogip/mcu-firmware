                    ...
                    motor_parameters.engine_thread_period_ms)
{
    if (control_mode_ == MotorControlMode::DUALPID_TRACKER) {
        ...
        if (motor_parameters.deceleration_filter_parameters) {
            tracker_deceleration_filter_ = new motion_control::DecelerationFilter(
                actuators::motor_tracker_deceleration_io_keys,
                *motor_parameters.deceleration_filter_parameters);
            tracker_meta_controller_.add_controller(tracker_deceleration_filter_);
        }
        if (motor_parameters.speed_limit_filter_parameters) {
            tracker_speed_limit_filter_ = new motion_control::SpeedLimitFilter(
                actuators::motor_tracker_speed_limit_io_keys,
                *motor_parameters.speed_limit_filter_parameters);
            tracker_meta_controller_.add_controller(tracker_speed_limit_filter_);
        }
        if (motor_parameters.acceleration_filter_parameters) {
            tracker_acceleration_filter_ = new motion_control::AccelerationFilter(
                actuators::motor_tracker_acceleration_io_keys,
                *motor_parameters.acceleration_filter_parameters);
            tracker_meta_controller_.add_controller(tracker_acceleration_filter_);
        }
