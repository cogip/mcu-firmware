...
public:
     ...
     */
    void set_position(const cogip::cogip_defs::Pose &pos)
    {
        pos_.x = pos.coords().x();
        pos_.y = pos.coords().y();
        pos_.O = pos.O();
    ...
    /**
     * @brief Get current position by copy usig cogip def format
     * 
     * @note Data units:
     *          - x, y: mm
     *          - O: rad
     * 
     * @return cogip::cogip_defs::Pose current position
     */
    cogip::cogip_defs::Pose position() const { return cogip::cogip_defs::Pose(pos_.x, pos_.y, pos_.O); }
    
    /**
     * @brief Get current polar velocities by copy usig cogip def format
     *
     * @note Data units:
     *          - linear: mm/period
     *          - angular: rad/period
     * 
     * @return cogip::cogip_defs::Polar current velocities
     */
    cogip::cogip_defs::Polar velocities() const { return cogip::cogip_defs::Polar(vel_.linear, vel_.angular); }
    ...
    /**
     * @brief update new robot pose (x, y, O) using segment approximation
     * 
