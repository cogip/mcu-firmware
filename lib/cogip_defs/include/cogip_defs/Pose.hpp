...
class Pose : public Coords
{
  public:
    ...
    /// Set 0-orientation.
    void set_O(float O ///< [in] new 0-orientation
    )
    {
        O_ = O;
    };
    ...
    bool operator==(const Pose other ///< [in] pose to compare
    ) const
    {
        return x_ == other.x_ && y_ == other.y_ && O_ == other.O_;
    };

    /// Copy data to Protobuf message.
    void pb_copy(PB_Pose& pose ///< [out] Protobuf message to fill
    ) const
    {
        pose.set_x(x_);
        pose.set_y(y_);
