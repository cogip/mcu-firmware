...
/// @file ParameterKeyHash.hpp
/// @brief Compile-time key hashing for parameters
///
/// Provides constexpr hash computation for parameter keys using FNV-1a algorithm.
...
///
/// @note This is a compile-time reimplementation of etl::fnv_1a_32.
...
///       - Offset basis: 0x811C9DC5 (2166136261u)
///       - Prime: 0x01000193 (16777619u)
...
constexpr uint32_t fnv1a_hash_bytes(const char* str, size_t len, uint32_t hash = 2166136261u)
{
    return (len == 0) ? hash : fnv1a_hash_bytes(str + 1, len - 1, (hash ^ static_cast<uint32_t>(*str)) * 16777619u);
}
...
/// @return The 32-bit hash value
///
/// Uses constexpr FNV-1a for compile-time hash computation.
