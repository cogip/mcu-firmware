...
class Parameter : public ParameterInterface<T>
{
  ...
  public:
    ...
    explicit Parameter(const T& initial_value) : value_{}, valid_(false), mutex_(MUTEX_INIT)
    {
        mutex_init(&mutex_);
        set(initial_value);
    }
    ...
    /// @brief Convert parameter to protobuf ParameterValue message
    /// @param message The ParameterValue message to convert to
    /// @return true if conversion succeeded
    ///
    /// @note Uses compile-time type dispatch to set the correct oneof field in ParameterValue
    /// message.
    bool pb_copy(ParameterValue& message) const override
    {
        bool result = false;
    ...
    /// @brief Convert parameter from protobuf ParameterValue message
    /// @param message The ParameterValue message to convert from
    /// @return true if conversion succeeded
    ///
    /// @note Uses compile-time type dispatch to get the correct oneof field from ParameterValue
    /// message.
    ///
    /// @note Validation is applied to the converted value.
    bool pb_read(const ParameterValue& message) override
    {
        T new_value;
        bool success = false;
