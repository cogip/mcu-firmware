...
class PID
{
  public:
    ...
    explicit PID(
        ...
        : kp_(kp), ki_(ki), kd_(kd), integral_term_(0), integral_term_limit_(integral_term_limit),
          previous_error_(0){};
    ...
    float integral_term() const
    {
        return integral_term_;
    };

    /// Return integral term limit.
    float integral_term_limit() const
    {
        return integral_term_limit_;
    };

    /// Set proportional gain.
    void set_kp(float kp ///< [in] new proportional gain
    )
    {
        kp_ = kp;
    };
    ...
    /// Set integral gain.
    void set_ki(float ki ///< [in] new integral gain
    )
    {
        ki_ = ki;
    };
    ...
    /// Set derivative gain.
    void set_kd(float kd ///< [in] new derivative gain
    )
    {
        kd_ = kd;
    };
    ...
    void set_integral_term_limit(float integral_term_limit ///< [in] new integral term limit
    )
    {
    ...
    void pb_read(const PB_Pid& pid ///< [in] Protobuf message to read
    )
    {
        kp_ = pid.get_kp();
        ki_ = pid.get_ki();
    ...
    void pb_copy(PB_Pid& pid ///< [out] Protobuf message to fill
    ) const
    {
        pid.set_kp(kp_);
        pid.set_ki(ki_);
