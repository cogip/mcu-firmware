...
public:
    /// @brief Constructor.
    /// @param parameters  Pointer to PID parameters (target_speed, signed_target_speed, …).
    /// @param keys        Pointer to a POD containing the three key names.
    explicit PassthroughPosePIDController(
        PassthroughPosePIDControllerParameters*     parameters,
        const PassthroughPosePIDControllerIOKeys*   keys
    )
        : Controller<PassthroughPosePIDControllerParameters,
                     PassthroughPosePIDControllerIOKeys>(parameters, keys) {}
