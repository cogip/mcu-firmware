...
///     - Tracks “dirty” keys on each call to `set()`. You can clear or take those dirty keys
///       to see exactly which parameters were modified since the last snapshot.
class ControllersIO {
public:
    /// @brief Compute FNV-1a 32-bit hash of a string key.
    /// @param key The parameter name (string_view).
    /// @return The 32-bit hash as ParamKey.
    static ParamKey hash_key(etl::string_view key);

    /// @brief Set or update a parameter value.
    /// @param key   The parameter name.
    /// @param value The value to store.
    /// @return 0 on success; EACCES if the key is read-only.
    /// @note If the key is marked read-only, the write is ignored and EACCES is returned.
    int set(etl::string_view key, const ParamValue& value);

    /// @brief Mark a parameter key as read-only.
    /// @param key The parameter name to protect.
    void mark_readonly(etl::string_view key);

    /// @brief Retrieve the raw variant value for a key.
    /// @param key The parameter name.
    /// @return An optional containing the `ParamValue` if found, or empty otherwise.
    OptionalValue get(etl::string_view key) const;
    ...
    template <typename T>
    etl::optional<T> get_as(etl::string_view key) const {
        if (auto opt = get(key)) {
    ...
    /// @brief Once you have run some controllers, you can call `snapshot_modified()` to get
    ///        all keys that were written since the last snapshot. Then call `clear_modified()`
    ///        to reset the “dirty” set.
    ///@{
    /// @brief Returns a vector of all ParamKeys that have been written since the last `clear_modified()`.
    etl::vector<ParamKey, MAX_PARAMS> snapshot_modified() const;
    ...
    static etl::vector<ParamKey, MAX_PARAMS>
    find_collisions(const etl::vector<ParamKey, MAX_PARAMS>& new_keys,
    ...
    static etl::vector<ParamKey, MAX_PARAMS>
    difference(const etl::vector<ParamKey, MAX_PARAMS>& after_ctrl,
               const etl::vector<ParamKey, MAX_PARAMS>& before_ctrl);

    /// @brief Clears the internal “dirty” set, so subsequent sets will be newly recorded.
    void clear_modified();
    ///@}
