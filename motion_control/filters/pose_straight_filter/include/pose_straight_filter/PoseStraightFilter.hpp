...
class PoseStraightFilter
    : public Controller<PoseStraightFilterParameters, PoseStraightFilterIOKeys>
{
public:
    /// @brief Constructor.
    /// @param parameters Pointer to movement switch thresholds.
    /// @param keys       Pointer to a POD containing all input and output key names.
    explicit PoseStraightFilter(
        PoseStraightFilterParameters*       parameters,
        const PoseStraightFilterIOKeys*     keys
    )
        : Controller<PoseStraightFilterParameters, PoseStraightFilterIOKeys>(parameters, keys)
    {
