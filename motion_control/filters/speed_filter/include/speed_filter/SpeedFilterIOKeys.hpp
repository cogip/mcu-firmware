...
struct SpeedFilterIOKeys {
    ...
    etl::string_view no_speed_filter;          ///< key for flag disabling filtering
    etl::string_view pose_reached_in;          ///< key for incoming pose‐reached status

    etl::string_view speed_error;              ///< key for computed speed error (filtered)
    etl::string_view pose_reached_out;         ///< key for outgoing pose‐reached status
};
} // namespace motion_control
