...
class SpeedFilter
    : public Controller<SpeedFilterParameters, SpeedFilterIOKeys>
{
public:
    /// @brief Constructor.
    /// @param parameters Pointer to filtering parameters (min/max speeds, acceleration, thresholds).
    /// @param keys       Pointer to a POD containing all input and output key names.
    explicit SpeedFilter(
        SpeedFilterParameters*       parameters,
        const SpeedFilterIOKeys*     keys
    )
        : Controller<SpeedFilterParameters, SpeedFilterIOKeys>(parameters, keys),
