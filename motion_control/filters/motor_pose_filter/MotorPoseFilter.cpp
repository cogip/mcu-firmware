...
void MotorPoseFilter::execute(ControllersIO& io)
{
    ...
    float abs_error = std::fabs(position_error);
    float threshold = this->parameters_->threshold();
    float decel = this->parameters_->deceleration();
    ...
    io.set(keys_->filtered_speed, std::fabs(target_speed));
