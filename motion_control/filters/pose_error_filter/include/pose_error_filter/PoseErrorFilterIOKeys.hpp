...
struct PoseErrorFilterIOKeys
{
    ...
    etl::string_view current_O;    ///< key for current orientation (linear mode for direction, angular mode for error)
    ...
    etl::string_view pose_reached; ///< key for pose reached status output (optional)
};
